...
# 52 "/opt/microchip/xc8/v2.10/pic/include/pic12f1840.h"
...
if(SD_Card_Command(0, 0x00000000) == 0x01) {
...
for(uint8_t i=250; i!=0; i--) {
uint8_t response = SD_Card_Command(41, 0x00000000);
if(response & 0x04) response = SD_Card_Command(1, 0x00000000);
if(response == 0x00) {
...
for(uint8_t i=250; i!=0; i--) {
if(SD_Card_Command(16, 512) == 0x00) {
...
SD_Card_ReadReg16(10, (uint8_t *)&SD_CID);
SD_Card_ReadReg16(9, (uint8_t *)&SD_CSD);
...
if(readOrWrite == 1) {
if(singleOrMultiBlock == 1) {
if(SD_Card_Command(25, addr) == 0x00) {
return 1;
}
} else {
if(SD_Card_Command(24, addr) == 0x00) {
SD_SPI_Write(0xFE);
return 1;
}
}
...
if(singleOrMultiBlock == 1) {
SD_Card_Command(18, addr);
return 1;
} else {
SD_Card_Command(17, addr);
SD_Card_WaitStartToken();
return 1;
}
}
...
uint16_t crc = 0;
if(SD_FLAGS.singleOrMultiBlock == 0) {
crc = SD_Card_ProcessCRC();
}


SD_Card_Command(((SD_FLAGS.readOrWrite == 1) ? 13 : 12), 0x00000000);
SD_Card_Disable();
return crc;
}

uint8_t SD_Card_ReadBlock(uint32_t addr, uint8_t *dst) {
if(SD_Card_RWInit(addr, 0, 0)) {
